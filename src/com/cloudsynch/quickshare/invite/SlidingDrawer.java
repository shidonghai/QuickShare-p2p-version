...
//        mHandle.setOnClickListener(new DrawerToggler());
        ...
        mTitle = findViewById(R.id.invite_title);
        new InviteManager().registerManager(this);
    }
...
//        Log.d("zxh", "onInterceptTouchEvent");
//        if (mLocked) {
//            return false;
//        }
//
...
//    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        Log.d("zxh", "onTouchEvent:" + mTracking);
        if (mLocked) {
            Log.d("zxh", "mLocked:true");
            return true;
        }

        final int action = event.getAction();
        Log.d("zxh", "action:" + action);
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                handle.getHitRect(frame);
                Log.d("zxh", "frame.contains((int) x, (int) y)" + frame.contains((int) x, (int) y));
                mTracking = frame.contains((int) x, (int) y) || mContent.getVisibility() == View.VISIBLE;
                Log.d("zxh", "ACTION_DOWN:" + mTracking);
                if (mTracking) {
                    handle.setPressed(true);
                    // Must be called before prepareTracking()
                    prepareContent();
            ...
            case MotionEvent.ACTION_MOVE:
                Log.d("zxh", "onTouchEvent ACTION_MOVE:" + mTracking);
            ...
            case MotionEvent.ACTION_CANCEL: {
                Log.d("zxh", "onTouchEvent ACTION_UP  ACTION_CANCEL:" + mTracking);
                if (mTracking) {
...
//                    velocity = MAXIMUM_MAJOR_VELOCITY;
                    ...
                    Log.d("zxh", "top:" + top);
                    final int left = mHandle.getLeft();
                    Log.d("zxh", "11111111111:" + mExpanded);
                    if (Math.abs(velocity) < mMaximumTapVelocity) {
                        if (vertical ? (mExpanded && top < mTapThreshold + mTopOffset) ||
                            ...
                            Log.d("zxh", "2222222222");
                            if (mAllowSingleTap) {
                                playSoundEffect(SoundEffectConstants.CLICK);
                                Log.d("zxh", "3333333333333");
                                if (mExpanded) {
                                    Log.d("zxh", "44444444444444");
                                    animateClose(vertical ? top : left);
                                } else {
                                    Log.d("zxh", "55555555555555");
                                    animateOpen(vertical ? top : left);
                                }
                            } else {
                                Log.d("zxh", "6666666666");
                                performFling(vertical ? top : left, velocity, false);
                            }

                        } else {
                            Log.d("zxh", "77777777777777");
                            if (mExpanded) {
                                animateClose(vertical ? top : left);
                            } else {
                                performFling(vertical ? top : left, velocity, false);
...
//        if (!content.isHardwareAccelerated()) content.buildDrawingCache();
   ...
   /* @Override
    ...
    private void closeDrawer() {
        ...
        mTitle.setBackgroundResource(R.drawable.invite_top);
        if (!mExpanded) {
            return;
        }
    ...
    private void openDrawer() {
        ...
        mTitle.setBackgroundResource(R.drawable.invite_top_down);
        if (mExpanded) {
            return;
        }
    ...
    private void toggleTitle() {
        mTitle.setBackgroundResource(mExpanded ? R.drawable.invite_top_down :
                R.drawable.invite_top);
    }
